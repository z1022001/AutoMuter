...
namespace AutoMuter
{
    ...
    public static class AudioUtilities
    {
        ...
        public static AudioSession GetProcessSession()
        {
            ...
            return GetProcessSession(id);
        }
        public static AudioSession GetProcessSession(Process p)
        {
            if (p == null) return null;

            foreach (AudioSession session in GetAllSessions())
            {
                if (session.ProcessId == p.Id || session.FileName == p.MainModule.FileName.ToString())
                    return session;
    ...
    public sealed class AudioSession : IDisposable
    {
        ...
        {
            AudioUtilities.ISimpleAudioVolume volume = _ctl as AudioUtilities.ISimpleAudioVolume;
            if (volume == null)
                return null;
            float level;
        ...
        public override string ToString()
        {
            ...
            //    ", Process: " + ((Process == null) ? "null" : Process.ProcessName) +
            //    ", FileName: " + ((Process == null) ? "null" : Process.MainModule.FileName) +
            //    ", DisplayName: " + (string.IsNullOrEmpty(str) ? "null" : str);
